...
def get_timeline():
    ...
    try:
        ...
        result = [
            {"hour": h, "total": v["total"], "threats": v["threats"]}
...
def get_threat_level():
    ...
    try:
        ...
        avg_score = sum(e.get("hybrid_score", 0) for e in recent_events) / len(
            recent_events
        )
    ...
    try:
        if request.method == "POST":
            ...
            if "adaptive_thresholds_enabled" in data:
                config["adaptive_thresholds_enabled"] = data[
...
def export_pdf():
    ...
    try:
        ...
        return (
            jsonify({"error": "reportlab not installed. Run: pip install reportlab"}),
            500,
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_logs():
    ...
    try:
        ...
        if not os.path.exists(filepath):
            return jsonify(
                {"lines": [], "total_lines": 0, "file": file_name, "truncated": False}
            )
        ...
        if request.method == "POST":
            ...
            if action == "add":
                ...
                if ip not in whitelist["whitelist_ips"]:
                    whitelist["whitelist_ips"].append(
                        {"ip": ip, "note": note, "added": datetime.now().isoformat()}
                    )
                ...
                whitelist["whitelist_ips"] = [
                    x for x in whitelist["whitelist_ips"] if x.get("ip") != ip
                ]
                save_json_file(WHITELIST_FILE, whitelist)
                return jsonify({"success": True})
        ...
        return jsonify(
            {
                ...
                "techniques_used": list(
                    set(e.get("mitre_technique_id", "") for e in ip_events)
                ),
                "timeline": [
                    {
                    ...
                    for e in sorted(
                        ip_events, key=lambda x: x["timestamp"], reverse=True
                    )
                ],
            }
        )
...
def get_stats():
    ...
    try:
        ...
        top_techniques = sorted(techniques.items(), key=lambda x: x[1], reverse=True)[
            :5
        ]
