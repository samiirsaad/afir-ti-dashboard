...
Write a 2-3 sentence professional security incident summary."""
    ...
    def generate_daily_summary(self, events):
        ...
        if threats > 0:
            avg_score = sum(e.get('hybrid_score', 0) for e in today_events if e.get('threat')) / threats
    ...
    def generate_compliance_report(self, standard):
        ...
        report = {
            ...
            'top_techniques': [],
            'compliance_status': 'COMPLIANT' if len(threats) > 0 else 'NO_THREATS_DETECTED'
        }
    ...
    def generate_compliance_pdf(self, standard):
        ...
        try:
            ...
            # Title
            elements.append(Paragraph(f"AFIR-TI Compliance Report - {standard.upper()}", styles['Title']))
            ...
            elements.append(Paragraph(f"Total Events Analyzed: {report.get('total_events', 0)}", styles['Normal']))
            elements.append(Paragraph(f"Threats Detected: {report.get('total_threats', 0)}", styles['Normal']))
            elements.append(Paragraph(f"IPs Blocked: {report.get('blocked_ips', 0)}", styles['Normal']))
            elements.append(Paragraph(f"Compliance Status: {report.get('compliance_status', 'UNKNOWN')}", styles['Normal']))
            elements.append(Spacer(1, 20))
            
            # Techniques table
            elements.append(Paragraph("Top MITRE ATT&CK Techniques", styles['Heading2']))
