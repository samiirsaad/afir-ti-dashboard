...
class CaseManager:
    ...
    def create_case(self, event):
        ...
        for case in cases:
            if case.get('src_ip') == src_ip and case.get('status') in ['open', 'investigating']:
    ...
    def reconstruct_incident(self, ip, from_ts=None, to_ts=None):
        ...
        if from_ts:
            try:
                ...
                ip_events = [e for e in ip_events 
                            if datetime.fromisoformat(e.get('timestamp', '').replace('Z', '+00:00')) >= from_dt]
            except:
                pass
        ...
        if to_ts:
            try:
                ...
                ip_events = [e for e in ip_events 
                            if datetime.fromisoformat(e.get('timestamp', '').replace('Z', '+00:00')) <= to_dt]
            except:
                pass
        ...
        first_alert = next((e.get('timestamp') for e in ip_events if e.get('threat')), None)
        ...
        return {
            ...
            'summary': {
                ...
                'phases_observed': list(set(e.get('attack_stage', '') for e in ip_events)),
