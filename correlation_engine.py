        ...
        for ip, ip_evts in ip_events.items():
            ...
            for i in range(len(ip_evts)):
                window_start = datetime.fromisoformat(
                    ip_evts[i].get("timestamp", "").replace("Z", "+00:00")
                )
                window_events = [ip_evts[i]]

                for j in range(i + 1, len(ip_evts)):
                    ts = datetime.fromisoformat(
                        ip_evts[j].get("timestamp", "").replace("Z", "+00:00")
                    )
                ...
                if len(window_events) >= 3:
                    ...
                    if corr_type:
                        combined_score = sum(
                            e.get("hybrid_score", 0) for e in window_events
                        ) / len(window_events)
                        ...
                        correlated.append(
                            {
                                ...
                                "events": [
                                    e.get("event_id", str(idx))
                                    for idx, e in enumerate(window_events)
                                ],
                            }
                        )
