...
class AdaptiveThresholdEngine:
    ...
    def analyze_and_adapt(self, events):
        ...
        high_fail_connections = sum(
            1
            for e in recent_events
            if e.get("features", {}).get("fail_count", 0) > 50 and not e.get("threat")
        )
    ...
    def reset_to_defaults(self):
        ...
        try:
            ...
            self._log_adaptation(
                "ALL", "various", "defaults", "Manual reset to defaults"
            )
            self.last_adaptation = None
