...
class AttackPrediction:
    ...
    def get_predictions(self):
        ...
        for ip, ip_evts in ip_events.items():
            ...
            avg_older = sum(scores[:5]) / min(5, len(scores[:-5])) if len(scores) > 5 else avg_recent
            ...
            if confidence > 0.5:  # Only show predictions with >50% confidence
                predictions.append({
                    ...
                    'trend': 'increasing' if trend > 0.1 else 'stable' if trend > -0.1 else 'decreasing',
    ...
    def _predict_attack_type(self, recent_events):
        ...
        # Analyze features
        avg_fail_count = sum(e.get('features', {}).get('fail_count', 0) for e in recent_events) / len(recent_events)
        avg_unique_ports = sum(e.get('features', {}).get('unique_ports', 0) for e in recent_events) / len(recent_events)
        avg_burst = sum(e.get('features', {}).get('burst_score', 0) for e in recent_events) / len(recent_events)
        avg_conn = sum(e.get('features', {}).get('conn_count', 0) for e in recent_events) / len(recent_events)
        
        # Determine likely attack type
        if avg_fail_count > 8:
            return "Brute force continuing", 0.75 + min(avg_fail_count / 50, 0.25), "pre-block"
        
        if avg_unique_ports > 15:
            return "Port scan likely", 0.7 + min(avg_unique_ports / 100, 0.3), "alert only"
        ...
        techniques = [e.get('mitre_technique_id', '') for e in recent_events if e.get('threat')]
        if techniques:
    ...
    def hunt(self, filters, time_range, sort_by, limit):
        ...
        for key, value in filters.items():
            if key == 'src_ip' and value:
                ...
                filtered = [e for e in filtered if e.get('hybrid_score', 0) >= float(value)]
            elif key == 'score_max':
                filtered = [e for e in filtered if e.get('hybrid_score', 0) <= float(value)]
                ...
                filtered = [e for e in filtered if value.lower() in e.get('reason', '').lower()]
