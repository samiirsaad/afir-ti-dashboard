...
Provide a concise, helpful answer in the same language as the question."""
    ...
    def _chat_fallback(self, message, events, blocked, stats):
        ...
        if any(
            ...
            for word in ["أخطر", "dangerous", "highest risk", "top ip"]
        ):
        ...
        if any(
            word in message_lower
            for word in ["وضع", "status", "summary", "عام", "general"]
        ):
            total = len(events)
    ...
    def _build_context(self, events, blocked, stats):
        ...
        for e in events[-10:]:
            context.append(
                f"  - {e.get('src_ip')} | Score: {e.get('hybrid_score', 0):.2f} | Threat: {e.get('threat')} | Technique: {e.get('mitre_technique_id', 'N/A')}"
            )
